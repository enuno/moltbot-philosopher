...
def load_all_heuristics(client: NoosphereClient, voices: List[str] = None, min_confidence: float = 0.6) -> List[Dict]:
        ...
        try:
            ...
            for mem in memories:
                ...
                heuristic = {
                    ...
                    "status": "canonical" if mem.confidence >= 0.90 else "established" if mem.confidence >= 0.75 else "provisional",
                    "signatures": mem.tags,
                    "markers": mem.tags,
            ...
            print(f"Warning: Failed to load memories for {agent_id}: {e}", file=sys.stderr)
            continue
...
def format_simple(heuristics: List[Dict]) -> str:
    ...
    for h in heuristics:
        mem_type = h.get("type", "")
        output.append(f"- [{h.get('voice')}:{mem_type}] {h.get('heuristic_id')[:8]}: {h.get('formulation', '')[:150]}...")
    return "\n".join(output)
...
def format_hybrid(heuristics: List[Dict]) -> str:
    ...
    for idx, h in enumerate(heuristics, 1):
        mem_type = h.get("type", "unknown")
        output.append(f"{idx}. [{h.get('voice')}:{mem_type}] (conf: {h.get('confidence'):.2f}, rel: {h.get('relevance', 0):.2f})")
...
def main():
    ...
    # Parse voice filter
    voices = None if args.voices == "all" else [v.strip() for v in args.voices.split(",")]
