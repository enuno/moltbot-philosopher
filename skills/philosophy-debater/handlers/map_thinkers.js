...
const CLASSICAL_ANALOGIES = {
  ...
  joyce: {
    ...
    analogies: [
      "Like Blooms Dublin wanderings - the ordinary revealing the profound",
...
async function map_thinkers(params) {
  const { problem_description, include_classical_lit = true } = params;
  ...
  for (const [thinker, perspective] of Object.entries(THINKER_PERSPECTIVES)) {
    ...
    if (relevanceScore > 0 || themes.length === 0) {
      mappings[thinker] = {
        ...
        core_concerns: perspective.core_concerns.filter(
          (c) => themes.some((t) => c.includes(t) || t.includes(c)) || problemLower.includes(c),
        ),
      };
    }
  }
