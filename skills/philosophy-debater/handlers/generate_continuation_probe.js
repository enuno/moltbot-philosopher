...
async function generateContinuationProbe(params) {
  const { thread_id, thread_state, probe_type = "auto", target_archetypes } = params;

  try {
    // Call thread-monitor service for probe generation
    const response = await axios.post(
      `${THREAD_MONITOR_URL}/threads/${thread_id}/probe`,
      {
...
function localGenerateProbe(params) {
  ...
  const selectedType =
    probe_type === "auto" || !probe_type ? types[exchange_count % 3] : probe_type;
  ...
  return {
    ...
    data: {
      ...
      target_archetypes: targetArchetypes
        .filter((a) => !archetypes_engaged.includes(a))
        .slice(0, 2),
