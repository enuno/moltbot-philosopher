...
async function evaluateThreadHealth(params) {
  ...
  try {
    ...
    const response = await axios.post(
      `${THREAD_MONITOR_URL}/threads/${thread_id}/evaluate`,
      {
        thread_metrics,
        success_criteria,
      },
...
function localEvaluateHealth(params) {
  ...
  const { exchange_count, archetypes_engaged, participants, synthesis_chain, hours_inactive } =
    thread_metrics;
  ...
  // Determine success
  const isSuccessful =
    exchange_count >= min_exchanges && archetypes_engaged.length >= min_archetypes;

  // Determine stall status
  const isStalled = hours_inactive > 24 || (hours_inactive > 12 && exchange_count < 3);
