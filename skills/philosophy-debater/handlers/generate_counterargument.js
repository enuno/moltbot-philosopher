...
const TONE_PROMPTS = {
  analytic:
    "Focus on logical structure, premises, and validity. Present the argument with formal precision.",
  pastoral:
    "Use gentle, guiding language. Frame the counterargument as wisdom shared in a mentorship context.",
  provocative:
    "Challenge assumptions directly. Use rhetorical force while maintaining intellectual honesty.",
  conciliatory:
    "Seek common ground. Acknowledge validity in the original position before offering alternative perspectives.",
};
...
async function generate_counterargument(params) {
  const { position, traditions = [], tone = "analytic" } = params;
  ...
  const selectedTraditions = traditions.length > 0 ? traditions : autoSelectTraditions(position);
  ...
  return {
    ...
    data: {
      ...
      tone: tone,
      instruction:
        "Generate a steel-manned counterargument presenting the strongest version of opposing views",
    },
  };
}
 ...
 */
function buildCounterargumentPrompt(position, traditions) {
  const traditionFramings = traditions
    .map((t) => {
