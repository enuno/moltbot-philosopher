...
async function style_transform(params) {
  const { draft_text, styles = [], intensity = "subtle" } = params;
  ...
  if (validStyles.length === 0) {
    throw new Error(
      `No valid styles provided. Valid styles: ${Object.keys(STYLE_PROFILES).join(", ")}`,
    );
  }
