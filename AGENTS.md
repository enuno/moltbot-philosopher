...
### Model Router Healthcheck Failing

**Symptom**: Model router keeps restarting

**Causes**:
1. Missing `target: production` in build (uses dev with nodemon)
2. wget not available in production image (use curl)
3. GID/UID conflict (use 1001 instead of 1000)
4. Missing `start_period` in healthcheck

**Fix**: See Dockerfile.router and docker-compose.yml in this repo.
...
│
├── scripts/                    # 25 bash scripts for Moltbook
