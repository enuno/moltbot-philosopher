...
class ScenarioDetector {
  ...
  async detectShallowResponse(comment) {
    ...
    const hasShallowIndicators = this.patterns.shallow.indicators.some((indicator) =>
      lowerComment.includes(indicator),
    );

    // Count philosophical terms
    const philoTerms = this.patterns.philosophicalTerms.filter((term) =>
      lowerComment.includes(term.toLowerCase()),
    );
    ...
    const confidence = detected ? (hasShallowIndicators ? 0.8 : 0.6) + (isShort ? 0.1 : 0) : 0;
  ...
  async detectAgreement(comment) {
    ...
    const agreementCount = this.patterns.agreement.indicators.filter((indicator) =>
      lowerComment.includes(indicator),
    ).length;
  ...
  async detectDisagreement(comment) {
    ...
    const disagreementCount = this.patterns.disagreement.indicators.filter((indicator) =>
      lowerComment.includes(indicator),
    ).length;
  ...
  async detectOffTopic(comment, thread) {
    ...
    const originalWords = new Set(thread.original_question.toLowerCase().split(/\s+/));
  ...
  async detectConflict(thread) {
    ...
    const detected = positions.length >= 2 && new Set(positions.map((p) => p.author)).size >= 2;
  ...
  async selectRelevantArchetypes(thread, scenario) {
    ...
    const scientificTopic = [
      ...
      "supernatural",
      "cosmos",
    ].some((term) => topic.includes(term));

    if (scientificTopic && selected.length < 3) {
      const skeptics = ["hitchens", "dawkins", "sagan", "feynman"].filter((a) =>
        unengaged.includes(a),
      );
