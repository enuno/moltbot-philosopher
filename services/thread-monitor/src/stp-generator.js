...
TARGET_ARCHETYPES: [comma-separated list of 2-3 archetypes to mention]`;
  ...
  parseStpResponse(content) {
    ...
    return {
      ...
      targetArchetypes: archetypesMatch ? archetypesMatch[1].split(",").map((s) => s.trim()) : [],
    };
  }
  ...
  assembleContent(stp, mentions, scenario) {
    const archetypeList = mentions.map((a) => a.charAt(0).toUpperCase() + a.slice(1)).join(" + ");
  ...
  buildContext(thread) {
    ...
    return recent
      .map((r) => `Exchange ${r.exchange_number} (${r.author}): ${r.synthesis}`)
      .join("\n");
  }
  ...
  generateFallback(thread, scenario) {
    const fallbacks = {
      standard: {
        ...
        propagation:
          "How might this framework account for counterexamples that challenge the core premise?",
      },
      shallow: {
        synthesis: "The stated position offers an initial intuition.",
        tension:
          "This creates tension with the need for epistemological grounding—what logical connective justifies this inference?",
        propagation:
          "Could you articulate whether you rely on modal entailment, probabilistic inference, or analogical reasoning?",
      },
      conflict: {
        synthesis: "We observe competing frameworks emerging.",
        tension:
          "This creates tension between the underlying ontological commitments each position presupposes.",
        propagation:
          "How might a third framework reconcile these competing intuitions without collapsing into relativism?",
      },
      silence: {
        synthesis: "The thread has reached a pause in the dialectic.",
        tension:
          "This creates tension with the expectation of continued philosophical exploration.",
        propagation:
          "Consider a counterfactual: if the opposite position were true, what would this imply for our original question?",
      },
    };
