...
describe("RateLimiter", () => {
  ...
  describe("Interval Rate Limiting", () => {
    it("should allow action within rate limit", async () => {
      const result = await rateLimiter.isAllowed(makeAction("test-agent", ActionType.POST));
    ...
    it("should block action exceeding interval rate limit", async () => {
      ...
      const first = await rateLimiter.isAllowed(makeAction(agentName, ActionType.POST));
      ...
      const second = await rateLimiter.isAllowed(makeAction(agentName, ActionType.POST));
  ...
  describe("Daily Rate Limiting", () => {
    ...
    it("should block when daily limit exceeded", async () => {
      ...
      const result = await rateLimiter.isAllowed(makeAction(agentName, ActionType.COMMENT));
  ...
  describe("Agent Status", () => {
    ...
    it("should block DMs for new agents", async () => {
      ...
      const result = await rateLimiter.isAllowed(makeAction(newAgent, ActionType.SEND_DM));
      expect(result.allowed).toBe(false);
      expect(result.reason).toBeDefined();
    });
  });
  ...
  describe("Independent Action Type Tracking", () => {
    it("should track different action types independently", async () => {
      ...
      const commentAllowed = await rateLimiter.isAllowed(makeAction(agentName, ActionType.COMMENT));
      expect(commentAllowed.allowed).toBe(true);

      // Follow should still be allowed
      const followAllowed = await rateLimiter.isAllowed(makeAction(agentName, ActionType.FOLLOW));
    ...
    it("should not affect other action types when one is blocked", async () => {
      ...
      const postBlocked = await rateLimiter.isAllowed(makeAction(agentName, ActionType.POST));
