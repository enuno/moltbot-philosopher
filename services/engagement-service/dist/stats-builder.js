...
exports.buildTrends = buildTrends;
exports.buildAgentsSection = buildAgentsSection;
exports.buildQualitySection = buildQualitySection;
exports.calculateRollingMetrics = calculateRollingMetrics;
/**
 * Build summary metrics across all agents
 * Handles missing/undefined states gracefully
 */
function buildSummary(agents, stateMap) {
    try {
        ...
        return {
            ...
            average_engagement_score: qualityScores.length > 0
...
function buildTrends(stateMap) {
    try {
        ...
        const topicTrends = Array.from(topicMentions.entries())
            .sort((a, b) => b[1] - a[1])
            .slice(0, 5)
            .map(([topicId, count]) => {
            ...
            return {
                ...
                trend_direction: totalPostsLast7 > totalPostsPrevious7
                    ? "up"
 ...
 */
function buildAgentsSection(agents, stateMap) {
    try {
        const agentMetrics = [];
        const today = new Date().toISOString().split("T")[0];
        for (const agent of agents || []) {
 ...
 */
function buildQualitySection(stateMap) {
    try {
        const qualityScores = [];
        const threadScores = [];
        ...
        const overallScore = qualityScores.length > 0 ? Math.round((qualityScores.reduce((a, b) => a + b, 0) / qualityScores.length) * 100) / 100 : 0;
        const threadAvg = threadScores.length > 0 ? Math.round((threadScores.reduce((a, b) => a + b, 0) / threadScores.length) * 100) / 100 : 0;
        const sentimentTrend = positiveCount > negativeCount * 1.2
            ? "positive"
 ...
 */
function calculateRollingMetrics(rollups) {
    try {
        ...
        const calculate = (days) => {
            try {
                ...
                const quality = days.length > 0
                    ? Math.round((days.reduce((sum, d) => sum + Math.max(0, Math.min(100, d?.averageQualityScore || 0)), 0) /
                        days.length) *
                        100) / 100
                    : 0;
