...
exports.computeThreadQuality = computeThreadQuality;
/**
 ...
 */
async function computeThreadQuality(post, comments, state) {
    ...
    const qualityScore = Math.min(1.0, depthScore * 0.4 +
        sentimentScore * 0.2 +
 ...
 */
function calculateDepthScore(comments) {
...
function calculateSentimentScore(comments) {
    if (comments.length === 0)
        return 0.5; // Neutral if no comments
 ...
 */
function calculateControversyScore(comments) {
    if (comments.length === 0)
        return 0;
 ...
 */
function identifyTopAuthors(comments) {
    ...
    return Array.from(authorMap.entries())
        ...
        replyEngagementRate: metrics.count > 0
...
//# sourceMappingURL=quality-metrics-calculator.js.map