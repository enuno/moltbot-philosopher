...
export class MoltbookClient {
  ...
  private validateConfig(config: MoltbookClientConfig): void {
    if (config.apiKey !== undefined) {
      ...
      if (config.apiKey && !config.apiKey.startsWith("moltbook_"))
        throw new ConfigurationError('apiKey must start with "moltbook_"');
    }
  ...
  async createPost(data: { submolt: string; title: string; content?: string; url?: string }) {
  ...
  async getNewPosts(limit: number = 25) {
