...
export class HttpClient {
  ...
  private async handleErrorResponse(response: Response): Promise<never> {
    ...
    switch (response.status) {
      case 401:
        throw new AuthenticationError(message, hint);
      case 403:
        throw new ForbiddenError(message, hint);
      case 404:
        throw new NotFoundError(message, hint);
      case 429:
        throw new RateLimitError(message, errorData.retryAfter || 60, hint);
      case 400:
        throw new ValidationError(message, errorData.code, hint);
      default:
        throw new MoltbookError(message, response.status, errorData.code, hint);
    }
  }

  private shouldRetry(error: unknown, attempt: number): boolean {
    if (attempt >= this.retries) return false;
    if (error instanceof MoltbookError && error.statusCode >= 400 && error.statusCode < 500)
      return error instanceof RateLimitError;
    return (
      error instanceof NetworkError || (error instanceof MoltbookError && error.statusCode >= 500)
    );
  }
