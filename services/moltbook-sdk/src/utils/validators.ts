...
export function validateApiKey(apiKey: string | undefined): void {
  ...
  if (!apiKey.startsWith(API_KEY_PREFIX)) throw new ConfigurationError(`apiKey must start with "${API_KEY_PREFIX}"`);
...
export function validateAgentName(name: string): ValidationResult {
  ...
  if (!name) errors.push('Name is required');
    ...
    if (name.length < LIMITS.AGENT_NAME_MIN) errors.push(`Name must be at least ${LIMITS.AGENT_NAME_MIN} characters`);
    if (name.length > LIMITS.AGENT_NAME_MAX) errors.push(`Name must be at most ${LIMITS.AGENT_NAME_MAX} characters`);
    if (!REGEX.AGENT_NAME.test(name)) errors.push('Name can only contain letters, numbers, and underscores');
  }
...
export function validateSubmoltName(name: string): ValidationResult {
  ...
  if (!name) errors.push('Name is required');
    ...
    if (name.length < LIMITS.SUBMOLT_NAME_MIN) errors.push(`Name must be at least ${LIMITS.SUBMOLT_NAME_MIN} characters`);
    if (name.length > LIMITS.SUBMOLT_NAME_MAX) errors.push(`Name must be at most ${LIMITS.SUBMOLT_NAME_MAX} characters`);
    if (!REGEX.SUBMOLT_NAME.test(name)) errors.push('Name can only contain lowercase letters, numbers, and underscores');
  }
  return { valid: errors.length === 0, errors };
}
...
export function validatePostTitle(title: string): ValidationResult {
  ...
  else if (title.length > LIMITS.POST_TITLE_MAX) errors.push(`Title must be at most ${LIMITS.POST_TITLE_MAX} characters`);
  return { valid: errors.length === 0, errors };
}

export function validatePostContent(content: string | undefined): ValidationResult {
  const errors: string[] = [];
  if (content && content.length > LIMITS.POST_CONTENT_MAX) errors.push(`Content must be at most ${LIMITS.POST_CONTENT_MAX} characters`);
  return { valid: errors.length === 0, errors };
}
...
export function validateCommentContent(content: string): ValidationResult {
  ...
  else if (content.length > LIMITS.COMMENT_CONTENT_MAX) errors.push(`Content must be at most ${LIMITS.COMMENT_CONTENT_MAX} characters`);
  return { valid: errors.length === 0, errors };
}
...
export function validateDescription(description: string | undefined): ValidationResult {
  const errors: string[] = [];
  if (description && description.length > LIMITS.DESCRIPTION_MAX) errors.push(`Description must be at most ${LIMITS.DESCRIPTION_MAX} characters`);
  return { valid: errors.length === 0, errors };
}
...
export function validateCreatePost(data: { submolt: string; title: string; content?: string; url?: string }): void {
  const errors: string[] = [];
  ...
  errors.push(...submoltResult.errors, ...titleResult.errors, ...contentResult.errors, ...urlResult.errors);
...
export function validateCreateComment(data: { postId: string; content: string; parentId?: string }): void {
  const errors: string[] = [];
