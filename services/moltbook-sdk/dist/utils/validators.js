...
function validateApiKey(apiKey) {
    ...
    if (apiKey.length < 25)
        throw new errors_1.ConfigurationError('apiKey is too short');
}
...
function validateCreatePost(data) {
    ...
    if (data.content && data.url)
        errors.push('Cannot have both content and URL');
    if (errors.length > 0)
        throw new errors_1.ValidationError(errors.join('; '), 'VALIDATION_ERROR');
}
...
function isValidApiKey(apiKey) {
    return typeof apiKey === 'string' && apiKey.startsWith(constants_1.API_KEY_PREFIX) && apiKey.length >= 25;
}
