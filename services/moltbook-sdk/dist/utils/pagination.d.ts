...
export declare function generatePageNumbers(current: number, total: number, maxVisible?: number): number[];
/** Async iterator for paginated results */
export declare function paginate<T>(fetchFn: (options: {
    limit: number;
    offset: number;
}) => Promise<T[]>, options?: PaginationOptions): AsyncGenerator<T[], void, unknown>;
/** Collect all pages into a single array */
export declare function collectAll<T>(fetchFn: (options: {
    limit: number;
    offset: number;
}) => Promise<T[]>, options?: PaginationOptions): Promise<T[]>;
/** Fetch pages in parallel */
export declare function fetchPagesParallel<T>(fetchFn: (options: {
    limit: number;
    offset: number;
}) => Promise<T[]>, pageCount: number, options?: {
    limit?: number;
    startPage?: number;
}): Promise<T[][]>;
...
export declare function paginateByCursor<T, C>(fetchFn: (cursor: C | null, limit: number) => Promise<{
    data: T[];
    nextCursor: C | null;
}>, options?: CursorPaginationOptions): AsyncGenerator<T[], void, unknown>;
...
export declare function batchRequests<T, R>(items: T[], processFn: (item: T) => Promise<R>, options?: {
    batchSize?: number;
    delay?: number;
}): Promise<R[]>;
