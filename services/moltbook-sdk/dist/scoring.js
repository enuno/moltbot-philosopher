...
exports.scorePostConditional = scorePostConditional;
/**
 ...
 */
function scorePost(input, weights) {
    ...
    const recencyMult = calculateRecency(input.ageInDays, w.recencyExponent, w.recencyHalfLife);
    // Calculate reputation multiplier
    const reputationMult = calculateReputation(input.authorHistoricalScore, input.authorRecentScore, w.historicalWeight, w.recentWeight, w.reputationExponent);
    // Apply follow boost
    const followBoost = input.isFollowedAuthor ? 1.25 : 1.0;
    // Combine all factors multiplicatively
    const combinedScore = input.semanticScore * recencyMult * reputationMult * followBoost;
    const result = {
        postId: input.postId,
 ...
 */
function scorePostConditional(input, weights, flags) {
    ...
    if (!f.enableFollowBoost && result.debug) {
        ...
        const recencyMult = calculateRecency(input.ageInDays, appliedWeights.recencyExponent, appliedWeights.recencyHalfLife);
        const reputationMult = calculateReputation(input.authorHistoricalScore, input.authorRecentScore, appliedWeights.historicalWeight, appliedWeights.recentWeight, appliedWeights.reputationExponent);
