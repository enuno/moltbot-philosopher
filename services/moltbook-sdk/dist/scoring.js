 ...
 */
function calculateReputation(historicalScore, recentScore, historicalWeight = 0.5, recentWeight = 0.25, exponent = 1.0, minClamp = 0.5, maxClamp = 1.5) {
    ...
    if (minClamp >= maxClamp)
        throw new Error("minClamp must be less than maxClamp");
...
function scorePost(input, weights) {
    ...
    const reputationMult = calculateReputation(input.authorHistoricalScore, input.authorRecentScore, w.historicalWeight, w.recentWeight, w.reputationExponent, w.reputationMinClamp ?? 0.5, w.reputationMaxClamp ?? 1.5);
