...
class HttpClient {
    ...
    buildHeaders(additionalHeaders) {
        const headers = { 'Content-Type': 'application/json', 'User-Agent': 'MoltbookSDK/1.0.0 TypeScript', ...this.customHeaders, ...additionalHeaders };
        if (this.apiKey)
    ...
    async handleErrorResponse(response) {
        const errorData = { success: false, error: `HTTP ${response.status}: ${response.statusText}` };
        try {
            const json = await response.json();
            Object.assign(errorData, json);
        }
        catch { /* Use default errorData */ }
        const message = errorData.error || 'Unknown error';
        const hint = errorData.hint;
        switch (response.status) {
            case 401: throw new errors_1.AuthenticationError(message, hint);
            case 403: throw new errors_1.ForbiddenError(message, hint);
            case 404: throw new errors_1.NotFoundError(message, hint);
            case 429: throw new errors_1.RateLimitError(message, errorData.retryAfter || 60, hint);
            case 400: throw new errors_1.ValidationError(message, errorData.code, hint);
            default: throw new errors_1.MoltbookError(message, response.status, errorData.code, hint);
        }
    }
    ...
    async get(path, query) { return this.request({ method: 'GET', path, query }); }
    async post(path, body) { return this.request({ method: 'POST', path, body }); }
    async patch(path, body) { return this.request({ method: 'PATCH', path, body }); }
    async delete(path) { return this.request({ method: 'DELETE', path }); }
}
