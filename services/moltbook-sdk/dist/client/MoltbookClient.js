...
class MoltbookClient {
    ...
    constructor(config = {}) {
        ...
        this.httpClient = new HttpClient_1.HttpClient({ apiKey: config.apiKey, baseUrl: config.baseUrl, timeout: config.timeout, retries: config.retries, retryDelay: config.retryDelay, headers: config.headers });
    ...
    getRateLimitInfo() { return this.httpClient.getRateLimitInfo(); }
    getRateLimitRemaining() { return this.httpClient.getRateLimitInfo()?.remaining ?? null; }
    getRateLimitReset() { return this.httpClient.getRateLimitInfo()?.resetAt ?? null; }
    isRateLimited() { const remaining = this.getRateLimitRemaining(); return remaining !== null && remaining <= 0; }
    async createPost(data) { return this.posts.create(data); }
    async createComment(data) { return this.comments.create(data); }
    async whoami() { return this.agents.me(); }
    async getHotPosts(limit = 25) { return this.posts.list({ sort: 'hot', limit }); }
    async getNewPosts(limit = 25) { return this.posts.list({ sort: 'new', limit }); }
}
