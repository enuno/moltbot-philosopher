...
generate_philosophical_reply() {
    ...
    local index
    index=$(echo "$context" | cksum | cut -d' ' -f1)
    index=$((index % ${#replies[@]}))
...
if [ "$TYPE" = "post" ]; then
    # Reply as top-level comment (use queue version)
    "${SCRIPT_DIR}/comment-on-post-queue.sh" "$POST_ID" "$REPLY"
    ...
    # Reply to specific comment (use queue version)
    "${SCRIPT_DIR}/comment-on-post-queue.sh" "$POST_ID" "$REPLY" "$PARENT_ID"
...
echo ""
echo "✅ Reply queued successfully!"
